...
var PublicApiTests = (function (_super) {
    __extends(PublicApiTests, _super);
    function PublicApiTests() {
        _super.apply(this, arguments);
    }
    /** Method called before the start of each test method */
    PublicApiTests.prototype.testInitialize = function () {
    ...
    /** Method called after each test method has completed */
    PublicApiTests.prototype.testCleanup = function () {
        this.useFakeServer = true;
        this.useFakeTimers = true;
    };
    PublicApiTests.prototype.registerTests = function () {
        var _this = this;
        ...
        var boilerPlateAsserts = function () {
            Assert.ok(_this.successSpy.called, "success");
            Assert.ok(!_this.errorSpy.called, "no error sending");
            var isValidCallCount = _this.loggingSpy.callCount === 0;
            Assert.ok(isValidCallCount, "logging spy was called 0 time(s)");
            if (!isValidCallCount) {
                while (_this.loggingSpy.args.length) {
                    Assert.ok(false, "[warning thrown]: " + _this.loggingSpy.args.pop());
                }
            }
        };
        var asserts = [];
        asserts.push(function () {
            var message = "polling: " + new Date().toISOString();
            Assert.ok(true, message);
            console.log(message);
            if (_this.successSpy.called) {
                boilerPlateAsserts();
                _this.testCleanup();
            }
            else if (_this.errorSpy.called || _this.loggingSpy.called) {
                boilerPlateAsserts();
            }
        });
        asserts.push(PollingAssert.createPollingAssert(function () {
            Assert.ok(true, "* checking success spy " + new Date().toISOString());
            return _this.successSpy.called;
        }, "sender succeeded"));
        this.testCaseAsync({
        ...
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    ...
                    for (var i = 0; i < 100; i++) {
                        _this.testAi.trackMetric("test" + i, Math.round(100 * Math.random()));
                    }
        ...
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    ...
                    Assert.ok(exception);
                    _this.testAi.trackEvent("test");
                    _this.testAi.trackException(exception);
                    _this.testAi.trackMetric("test", Math.round(100 * Math.random()));
                    _this.testAi.trackTrace("test");
                    _this.testAi.trackPageView();
                }
            ].concat(asserts)
