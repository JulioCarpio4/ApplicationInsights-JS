...
var SnippetTests = (function (_super) {
    __extends(SnippetTests, _super);
    function SnippetTests() {
        _super.apply(this, arguments);
        this.aiName = "appInsights";
        this.instrumentationKey = "3e6a441c-b52b-4f39-8944-f81dd6c2dc46";
        ...
        this.queueCallCount = 100;
    }
    SnippetTests.prototype.loadSnippet = function (path) {
    ...
    /** Method called before the start of each test method */
    SnippetTests.prototype.testInitialize = function () {
        ...
        window['queueTest'] = function () { return null; };
    ...
    /** Method called after each test method has completed */
    SnippetTests.prototype.testCleanup = function () {
        this.useFakeServer = true;
        this.useFakeTimers = true;
        window[this.aiName] = this.originalAppInsights;
    };
    SnippetTests.prototype.registerTests = function () {
        var _this = this;
        ...
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    _this.loadSnippet(snippet_01);
                },
                function () {
                    _this.senderMocks = _this.setAppInsights();
                    window["appInsights"].trackPageView("test", { property: "p1" }, { measurement: 5 });
                }]
                .concat(this.waitForResponse())
                .concat(function () {
                Assert.equal(_this.queueCallCount, _this.queueSpy.callCount, "queue is emptied");
                Assert.equal(1, _this.senderMocks.sender.callCount, "v2 send called 1 time");
                _this.boilerPlateAsserts(_this.senderMocks);
                // check url and properties
                var pv = _this.senderMocks.sender.args[0][0].data.baseData;
        ...
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    _this.loadSnippet(snippet_Latest);
                },
                function () {
                    _this.senderMocks = _this.setAppInsights();
                    window["appInsights"].trackPageView("test", { property: "p1" }, { measurement: 5 });
                ...
                .concat(function () {
                Assert.equal(_this.queueCallCount, _this.queueSpy.callCount, "queue is emptied");
                Assert.equal(1, _this.senderMocks.sender.callCount, "v2 send called");
                _this.boilerPlateAsserts(_this.senderMocks);
                // check url and properties
                ...
                var data = _this.senderMocks.sender.args[0][0];
    ...
    SnippetTests.prototype.testSnippet = function (snippetPath) {
        var _this = this;
        var delay = 250;
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    _this.loadSnippet(snippetPath);
                },
                function () {
                    Assert.ok(window[_this.aiName], _this.aiName + " is loaded");
                }
            ]
        });
        this.testCaseAsync({
            ...
            steps: [
                ...
                .concat(PollingAssert.createPollingAssert(function () {
                return (!window[_this.aiName].hasOwnProperty("queue"));
            }, "waiting for AI Init() to finish" + new Date().toISOString(), 5, 200))
                .concat(function () {
                Assert.ok(!window[_this.aiName].hasOwnProperty("queue"), "queue was removed during the init");
                Assert.equal(_this.queueCallCount, _this.queueSpy.callCount, "should drain the queue");
            })
        });
        this.testCaseAsync({
        ...
        this.testCaseAsync({
            ...
            steps: [
                ...
                function () {
                    _this.senderMocks = _this.setAppInsights();
                    window[_this.aiName].trackEvent("test");
                    window[_this.aiName].trackException(new Error("oh no!"));
                    window[_this.aiName].trackMetric("test", Math.round(100 * Math.random()));
                    window[_this.aiName].trackTrace("test");
                    window[_this.aiName].trackPageView("test page");
                }]
                .concat(this.waitForResponse())
                .concat(function () {
                Assert.equal(_this.queueCallCount, _this.queueSpy.callCount, "queue is emptied");
                Assert.equal(5, _this.senderMocks.sender.callCount, "send called 5 times");
                _this.boilerPlateAsserts(_this.senderMocks);
            })
        });
    };
    SnippetTests.prototype.waitForResponse = function () {
        var _this = this;
        return PollingAssert.createPollingAssert(function () {
            return (_this.senderMocks.successSpy.called || _this.senderMocks.errorSpy.called);
        }, "Wait for response" + new Date().toISOString(), 5, 1000);
    };
    SnippetTests.prototype.checkConfig = function () {
        var _this = this;
        var initial = window[this.aiName];
        var test = function (expected, memberName, readFunction) {
            var appIn = window[_this.aiName];
            appIn.config[memberName] = expected;
            var actual = readFunction();
            Assert.equal(expected, actual, memberName + ": value is read dynamically");
        };
        var testSenderValues = function (expected, memberName) {
            var appIn = window[_this.aiName];
            test(expected, memberName, appIn.context._sender._config[memberName]);
        };
        var testContextValues = function (expected, memberName) {
            var appIn = window[_this.aiName];
    ...
    SnippetTests.prototype.boilerPlateAsserts = function (spies) {
