...
var PerformanceTestHelper = (function (_super) {
    __extends(PerformanceTestHelper, _super);
    function PerformanceTestHelper(timeout) {
        _super.call(this);
        this.testCount = 0;
    ...
    /** Method called before the start of each test method */
    PerformanceTestHelper.prototype.testInitialize = function () {
        ...
        this.appInsights.context._sender._sender = function () { return null; };
    ...
    /** Method called after each test method has completed */
    PerformanceTestHelper.prototype.testCleanup = function () {
        this.useFakeServer = true;
        this.useFakeTimers = true;
        var serializedPerfResults = window["perfResults"] || "[]";
        var perfResults = (JSON.parse(serializedPerfResults));
    ...
    PerformanceTestHelper.prototype.toCsv = function (array) {
        var headers = "";
        if (array.length > 0) {
            var names = [];
    ...
    PerformanceTestHelper.prototype.enqueueTest = function (name, action) {
        JSLitmus.test(name, function (count) {
            while (count--) {
                action();
            }
        });
    };
    PerformanceTestHelper.prototype.runTests = function () {
        var _this = this;
        JSLitmus.runAll().done(function () { return _this.onTestsComplete(); });
    };
    PerformanceTestHelper.prototype.onTestsComplete = function () {
        var perfLogging = new Microsoft.ApplicationInsights.AppInsights({
        ...
        perfLogging.context._sender._sender = function (payload) {
        ...
        for (var i = 0; i < JSLitmus._tests.length; i++) {
            ...
            var data = new Microsoft.ApplicationInsights.Telemetry.Common.Data(Microsoft.ApplicationInsights.Telemetry.Event.dataType, event);
    ...
    PerformanceTestHelper.prototype.onTimeout = function () {
        if (!this.isDone) {
     ...
     */
    PerformanceTestHelper.prototype.synchronouslyLoadJquery = function () {
        if (!window["$"]) {
...
var ApiPerfTests = (function (_super) {
    __extends(ApiPerfTests, _super);
    function ApiPerfTests() {
        _super.apply(this, arguments);
    }
    ApiPerfTests.prototype.registerTests = function () {
        var _this = this;
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    ...
                    // appInsights
                    _this.enqueueTest("appInsights.trackEvent", function () { return _this.appInsights.trackEvent("ev", _this.testProperties, _this.testMeasurements); });
                    _this.enqueueTest("appInsights.trackException", function () { return _this.appInsights.trackException(error, null, _this.testProperties, _this.testMeasurements); });
                    _this.enqueueTest("appInsights.trackMetric", function () { return _this.appInsights.trackMetric("m", 1, _this.testProperties, _this.testMeasurements); });
                    _this.enqueueTest("appInsights.trackPageView", function () { return _this.appInsights.trackPageView("name", "url", _this.testProperties, _this.testMeasurements); });
                    _this.enqueueTest("appInsights.trackTrace", function () { return _this.appInsights.trackTrace("t", _this.testProperties, _this.testMeasurements); });
                    // context
                    ...
                    _this.enqueueTest("appInsights.context.track", function () { return _this.appInsights.context.track(envelope); });
                    _this.runTests();
                },
                function () { return _this.onTimeout(); }
            ]
        });
    };
    return ApiPerfTests;
...
var SerializerPerfTests = (function (_super) {
    __extends(SerializerPerfTests, _super);
    function SerializerPerfTests() {
        _super.apply(this, arguments);
    }
    SerializerPerfTests.prototype.registerTests = function () {
        var _this = this;
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    ...
                    var pageViewPropsMeas = new Microsoft.ApplicationInsights.Telemetry.PageView("page name", undefined, _this.testProperties, _this.testMeasurements);
                    var pageViewPerformancePropsMeas = new Microsoft.ApplicationInsights.Telemetry.PageViewPerformance("page name", undefined, 42, _this.testProperties, _this.testMeasurements);
                    var exceptionPropsMeas = new Microsoft.ApplicationInsights.Telemetry.Exception(error, "unhandled", _this.testProperties, _this.testMeasurements);
                    var eventPropsMeas = new Microsoft.ApplicationInsights.Telemetry.Event("event name", _this.testProperties, _this.testMeasurements);
                    var tracePropsMeas = new Microsoft.ApplicationInsights.Telemetry.Trace("this is a trace", _this.testProperties);
                    _this._enqueueTest("serializer.serialize(pageView)", pageView);
                    _this._enqueueTest("serializer.serialize(pageViewPerformance)", pageViewPerformance);
                    _this._enqueueTest("serializer.serialize(metric)", metric);
                    _this._enqueueTest("serializer.serialize(exception)", exception);
                    _this._enqueueTest("serializer.serialize(event)", event);
                    _this._enqueueTest("serializer.serialize(trace)", trace);
                    _this.runTests();
                },
                function () { return _this.onTimeout(); }
            ]
    ...
    SerializerPerfTests.prototype._enqueueTest = function (name, telemetry) {
        //telemetry.iKey = "fakeIKey";
        this.enqueueTest(name, function () { return Microsoft.ApplicationInsights.Serializer.serialize(telemetry); });
    };
    return SerializerPerfTests;
}(PerformanceTestHelper));
new SerializerPerfTests().registerTests();
/// <reference path="../testframework/performancetesthelper.ts" />
var PageLoadPerfTests = (function (_super) {
    __extends(PageLoadPerfTests, _super);
    function PageLoadPerfTests() {
        _super.apply(this, arguments);
    }
    /** Method called before the start of each test method */
    PageLoadPerfTests.prototype.testInitialize = function () {
        this.useFakeTimers = false;
        this.clock.restore();
        this.errorSpy = this.getListener();
    };
    PageLoadPerfTests.prototype.registerTests = function () {
        var _this = this;
        ...
        this.testCaseAsync({
            ...
            steps: [
                function () {
                    ...
                    _this.loadPageToIFrame("testPageNoAppInsights");
                },
                function () {
                    _this.errorSpy.reset();
                    _this.loadPageToIFrame("testPageNoAppInsights");
                }
            ].concat(this.poll(function () {
                if (_this.errorSpy.called) {
                    var timing = JSON.parse(_this.errorSpy.args[0][0].data);
                    results.push({
        ...
        this.testCaseAsync({
            ...
            steps: [
                ...
                function () {
                    _this.errorSpy.reset();
                    _this.loadPageToIFrame("testPageWithAppInsights");
                }
            ].concat(this.poll(function () {
                if (_this.errorSpy.called) {
                    var timing = JSON.parse(_this.errorSpy.args[0][0].data);
    ...
    PageLoadPerfTests.prototype.poll = function (func, count) {
        if (count === void 0) { count = 50; }
        var polling = [];
        if (typeof window != "undefined" && window.performance && window.performance.timing) {
            for (var i = 0; i < count; i++) {
                polling.push(function () {
                    if (func()) {
    ...
    PageLoadPerfTests.prototype.detectPlatformAndOs = function () {
    ...
    PageLoadPerfTests.prototype.getListener = function () {
        var listener = { onMessage: function (data) { return null; } };
    ...
    PageLoadPerfTests.prototype.loadPageToIFrame = function (path) {
